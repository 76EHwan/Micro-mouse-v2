...
__STATIC_INLINE void LSM6DS3TR_C_ReadReg(uint8_t reg_addr, uint8_t *rx_byte,
	...
	HAL_SPI_Transmit(LSM6DS3TR_SPI_PORT, &tx_byte, 1, HAL_MAX_DELAY);   // 주소 전송
	HAL_SPI_Receive(LSM6DS3TR_SPI_PORT, rx_byte, size, HAL_MAX_DELAY); // 데이터 수신
...
void LSM6DS3TR_C_CheckCTRL() {
	...
	if (read_ctrl3 == CTRL3_C) {
		Custom_LCD_Printf(0, 0, "CTRL3 OK 	  0x%02X", read_ctrl3);
	} else {
		Custom_LCD_Printf(0, 0, "CTRL3_C ERR  0x%02X", read_ctrl3);
	}
	HAL_Delay(200);
	if (read_ctrl1 == CTRL1_XL) {
		Custom_LCD_Printf(0, 1, "CTRL1 OK 	  0x%02X", read_ctrl1);
	} else {
		Custom_LCD_Printf(0, 1, "CTRL1_XL ERR 0x%02X", read_ctrl1);
	}
	HAL_Delay(200);
	if (read_ctrl2 == CTRL2_G) {
		Custom_LCD_Printf(0, 2, "CTRL2 OK 	  0x%02X", read_ctrl2);
	} else {
		Custom_LCD_Printf(0, 2, "CTRL2_XL ERR 0x%02X", read_ctrl2);
	}
	HAL_Delay(200);
	if (read_ctrl4 == CTRL4_C) {
		Custom_LCD_Printf(0, 3, "CTRL4 OK 	  0x%02X", read_ctrl4);
	} else {
		Custom_LCD_Printf(0, 3, "CTRL4_C ERR  0x%02X", read_ctrl4);
	}
	HAL_Delay(200);
	if (read_ctrl5 == CTRL5_C) {
		Custom_LCD_Printf(0, 4, "CTRL5 OK 	  0x%02X", read_ctrl5);
	} else {
		Custom_LCD_Printf(0, 4, "CTRL5_XL ERR 0x%02X", read_ctrl5);
	}
	HAL_Delay(2000);
	Custom_LCD_Clear();
	if (read_ctrl6 == CTRL6_C) {
		Custom_LCD_Printf(0, 0, "CTRL6 OK 	  0x%02X", read_ctrl6);
	} else {
		Custom_LCD_Printf(0, 0, "CTRL6_XL ERR 0x%02X", read_ctrl6);
	}
	HAL_Delay(200);
	if (read_ctrl7 == CTRL7_G) {
		Custom_LCD_Printf(0, 1, "CTRL7 OK 	  0x%02X", read_ctrl7);
	} else {
		Custom_LCD_Printf(0, 1, "CTRL7_C ERR  0x%02X", read_ctrl7);
	}
	HAL_Delay(200);
	if (read_ctrl8 == CTRL8_XL) {
		Custom_LCD_Printf(0, 2, "CTRL8 OK 	  0x%02X", read_ctrl8);
	} else {
		Custom_LCD_Printf(0, 2, "CTRL8_XL ERR 0x%02X", read_ctrl5);
	}
	HAL_Delay(200);
	if (read_ctrl9 == CTRL9_XL) {
		Custom_LCD_Printf(0, 3, "CTRL9 OK 	  0x%02X", read_ctrl9);
	} else {
		Custom_LCD_Printf(0, 3, "CTRL9 ERR 	  0x%02X", read_ctrl9);
	}
	HAL_Delay(200);
	if (read_ctrl10 == CTRL10_C) {
		Custom_LCD_Printf(0, 4, "CTRL10 OK 	  0x%02X", read_ctrl10);
	} else {
		Custom_LCD_Printf(0, 4, "CTRL10_C ERR 0x%02X", read_ctrl10);
	}
	HAL_Delay(2000);
	Custom_LCD_Clear();
}
